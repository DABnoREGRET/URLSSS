...
public class UrlService : IUrlService
{
    ...
    public async Task<bool> DeleteUrlAsync(string shortCode)
    {
        try
        {
            ...
            if (shortenedUrl == null)
            {
                _logger.LogWarning("Attempted to delete non-existent or inactive URL: {ShortCode}", shortCode);
                return false;
            }

            // Deactivate the URL instead of deleting it to preserve analytics
            shortenedUrl.IsActive = false;
            await _context.SaveChangesAsync();

            // Remove from cache (non-blocking, continue even if cache removal fails)
            try
            {
                var cacheKey = $"url:{shortCode}";
                await _cacheService.RemoveAsync(cacheKey);
                _logger.LogDebug("Successfully removed URL from cache: {ShortCode}", shortCode);
            }
            catch (Exception cacheEx)
            {
                _logger.LogWarning(cacheEx, "Failed to remove URL from cache: {ShortCode}. Continuing with deletion.", shortCode);
                // Continue with deletion even if cache removal fails
            }

            _logger.LogInformation("Successfully deactivated URL: {ShortCode}", shortCode);
            return true;
        }
        catch (Exception ex)
        {
